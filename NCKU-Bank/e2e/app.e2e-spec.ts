...
describe('Starting tests for NCKU-Bank', function() {
  ...
  it('website title should be NCKU-Bank', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('NCKU-Bank');
    })
  });
  ...
  it('navbar-brand should be NCKU-Bank',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('NCKU-Bank');
    });
  });
